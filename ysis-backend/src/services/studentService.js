 ...
 * affected means the record changed underneath the client — that IS
 * the conflict signal, not something the app has to separately notice.
 *
 * Returns { updated: true, student } on success,
 *         { updated: false } if a conflict was detected (caller inserts
 *         into `conflicts`).
 *
 * `allowedFields` is normally EDITABLE_FIELDS. Protected workflows such
 * as promotion may explicitly provide their own small allow-list.
 */
async function applyConditionalUpdate({
  ...
  const setFields = Object.keys(changes).filter((k) =>
