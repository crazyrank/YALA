...
/**
 * Read the photo from disk only when we are about to send it.
 * This keeps the large base64 out of the long-lived SQLite payload.
 */
async function hydratePhotoPayload(payload) {
  if (payload.localUri && !payload.imageBase64) {
    try {
      const imageBase64 = await FileSystem.readAsStringAsync(payload.localUri, {
        encoding: FileSystem.EncodingType.Base64,
      });
      return { ...payload, imageBase64 };
    } catch (e) {
      console.warn('Failed to read photo for sync:', payload.localUri, e);
      throw e;
    }
  }
  return payload;
...
export async function triggerSync() {
  ...
  try {
    ...
    // Build operations and hydrate photos only at send time
    const operations = [];
    for (const row of pending) {
      let payload = JSON.parse(row.payload);
      if (row.op_type === 'upload_photo') {
        payload = await hydratePhotoPayload(payload);
      }
      operations.push({
