...
function MenuRow({ icon, label, subtitle, color, onPress, danger }) {
  ...
  return (
    ...
    >
      <View style={[styles.iconBox, { backgroundColor: danger ? colors.errorBg : color + '18' }]}>
        <Ionicons name={icon} size={20} color={danger ? colors.error : color} />
      </View>
      <View style={styles.rowText}>
        <Text style={[styles.rowLabel, { color: danger ? colors.error : colors.textPrimary }]}>
          {label}
        </Text>
        {!!subtitle && (
...
export default function MoreScreen({ navigation }) {
  ...
  return (
    ...
    >
      ...
      <Text style={[styles.section, { color: colors.textMuted, marginTop: 18 }]}>ACCOUNT</Text>
